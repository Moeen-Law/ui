...
export function FAQ() {
    ...
    return (
        <section
            id="faq"
            className="content-visibility-auto relative scroll-mt-24 overflow-hidden bg-muted/50 px-6 py-24 md:px-8"
        >
            ...
            <div className="relative mx-auto grid max-w-[1280px] grid-cols-1 gap-10 lg:grid-cols-[minmax(280px,0.82fr)_minmax(0,1.18fr)] lg:items-start lg:gap-14">
                <div className="flex flex-col gap-7 text-center lg:sticky lg:top-28 lg:text-start">
                    <div>
                        <span className="inline-flex w-fit items-center rounded-full border border-blue-500/20 bg-blue-500/10 px-4 py-1.5 font-sans text-sm font-extrabold text-blue-500 shadow-[0_10px_30px_rgba(59,130,246,0.08)]">
                            {getFAQText(faq.label, fallbackFAQ.label)}
                        </span>
                        <h2 className="mt-5 bg-linear-to-br from-foreground via-foreground to-blue-500 bg-clip-text font-sans text-4xl font-black leading-tight text-transparent md:text-5xl">
                        ...
                        <p className="mt-6 font-sans text-lg leading-8 text-muted-foreground md:text-xl">
                    ...
                    <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-5 text-start shadow-[0_20px_40px_rgba(59,130,246,0.08)] backdrop-blur md:p-6">
                        ...
                        <h3 className="font-sans text-xl font-black text-foreground">
                            {getFAQText(faq.supportTitle, fallbackFAQ.supportTitle)}
                        </h3>
                        <p className="mt-3 font-sans text-sm leading-7 text-muted-foreground md:text-base">
                            {getFAQText(faq.supportText, fallbackFAQ.supportText)}
                        </p>
                        <Button
                            type="button"
                            onClick={handleStart}
                            className="mt-5 h-11 w-full cursor-pointer rounded-xl bg-blue-500 px-6 font-sans font-bold text-white shadow-[0_10px_30px_rgba(59,130,246,0.32)] transition-all hover:-translate-y-0.5 hover:bg-blue-400 hover:shadow-[0_14px_34px_rgba(59,130,246,0.38)] sm:w-auto"
                        >
                ...
                >
                    {items.map((item, index) => (
                        ...
                        >
                            <AccordionTrigger className="cursor-pointer gap-4 py-5 font-sans text-base font-extrabold leading-7 text-foreground hover:no-underline md:py-6 md:text-lg [&_[data-slot=accordion-trigger-icon]]:mt-1 [&_[data-slot=accordion-trigger-icon]]:size-5 [&_[data-slot=accordion-trigger-icon]]:text-blue-500">
                                {item.question}
                            </AccordionTrigger>
                            <AccordionContent className="pb-5 pe-8 font-sans text-sm leading-7 text-muted-foreground md:pb-6 md:text-base md:leading-8">
                                {item.answer}
                            </AccordionContent>
                        </AccordionItem>
                    ))}
